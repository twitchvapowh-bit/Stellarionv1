/* STELLARION 1.6.34 — Anti-duplication de flotte (ledger) + outil de réparation.
   ...
   OUTILS CONSOLE :
   - stellarionShipAudit1636()              : état des stocks + registre
   - await stellarionReduceShips1636(0.2)   : garde 20% des vaisseaux côté serveur
*/
(function () {
  "use strict";
  if (window.__stellarionFleetLedger1636) return;
  window.__stellarionFleetLedger1636 = true;

  var KEY = "stellarion_fleet_ledger_1636";
  ...
  function wrapProcessFleets() {
    if (typeof window.processFleets !== "function" || window.processFleets.__ledgerGuarded1636) return;
    var original = window.processFleets;

    var wrapped = function () {
      // 1.6.36 — garde anti-récursion : certains anciens wrappers processFleets
      // peuvent former une boucle (V7 -> 1605 -> 1636 -> V7).
      // Si on ré-entre pendant le même cycle, on coupe l'appel pour éviter
      // Maximum call stack size exceeded, sans recréditer de flotte.
      if (window.__stellarionProcessFleetsLedger1636Busy) return;
      window.__stellarionProcessFleetsLedger1636Busy = true;
      try {
        var st = S();
          ...
          st.fleets.forEach(function (f) {
            ...
            if (!entry) {
              ...
              if (f.ships && Object.keys(f.ships).length) {
                console.warn("[fleet-ledger-1636] Crédit fantôme bloqué pour", f.id, f.ships);
                f.ships = {};
              }
              return;
            }
      ...
      try {
        ...
        window.__stellarionProcessFleetsLedger1636Busy = false;
      }
    };

    wrapped.__ledgerGuarded1636 = true;
    // On propage TOUS les marqueurs connus pour empêcher les anciens patchs
    // de ré-emballer processFleets en boucle.
    wrapped.__creditGuarded1605 = true;
    wrapped.__serverAuthGuarded1606 = true;
    if (original.__serverCombatGuardV7) wrapped.__serverCombatGuardV7 = true;
    if (original.__v7ServerCombatEco) wrapped.__v7ServerCombatEco = true;
  ...
  window.stellarionShipAudit1636 = function () {
    var st = S();
    var perPlanet = {};
    var total = 0;
  ...
  function localReduceShips(factor) {
    ...
    if (!(factor > 0 && factor < 1)) return "Usage : stellarionReduceShips1636(0.2) pour garder 20% des vaisseaux.";
    var st = S();
  ...
  async function serverReduceShips(factor) {
    factor = Number(factor);
    if (!(factor > 0 && factor < 1)) return "Usage : await stellarionReduceShips1636(0.2) pour garder 20% des vaisseaux.";
    if (typeof window.stellarionServerAuthorityAction1570 !== "function") {
      return localReduceShips(factor);
    }
    var before = window.stellarionShipAudit1636();
    var out = null;
    try {
      ...
      return {
        ...
        note: "Redeploie la fonction Supabase game-action, puis relance await stellarionReduceShips1636(" + factor + ")."
      };
    }
    try {
      var st = S();
    ...
    try { if (typeof window.save === "function") window.save(); } catch (e) {}
    try { if (typeof window.render === "function") window.render(); } catch (e) {}
    var after = window.stellarionShipAudit1636();
    return {
      mode: "server-authority",
      beforeLocalAudit: before,
  ...
  window.stellarionReduceShips1636 = function (factor) {
